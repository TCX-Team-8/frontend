...
const Liste_taches = () => {
    ...
    const TacheTable: React.FC<TacheTableProps> = ({
      ...
      <div className="w-full flex flex-col">
        {isFilterOpen && (
          <FilterOptions filterOptions={filterOptions} toggleFilterOption={toggleFilterOption} />
        )}
        <TableHeader isFilterOpen={isFilterOpen} setIsFilterOpen={setIsFilterOpen} />
        {Taches.length === 0 ? (
          <h2 className="text-black text-center">No results found</h2>
      ) : (
          Taches.map((Tache,index) => <TacheRow key={index} index={index} Tache={Tache} />)
        )}
      </div>
    );
      ...
      <div className="flex gap-4 p-4 flex-wrap">
        {Object.keys(filterOptions).map((option) => (
          <div
            ...
            onClick={() => toggleFilterOption(option as keyof typeof FilterOptions)}
    ...
    const TacheRow: React.FC<TacheRowProps> = ({ Tache,index }) => (
        <div className={`w-[80vw] flex place-content-between text-black gap-6 items-center h-16  p-2  ${index%2==0?"bg-[#F8F9FC]":"bg-[#FFFFFF]"}`}>
