...
import Overview from "./Components/GGeneral-overview";
...
export default function Global_Dashboard() {
  ...
  useEffect(() => {
    // Actual fetching of data from an API
            ...
            {COLORS.map((item, index) => (
              <li key={index}>
                <div
                  ...
                  className="w-20 rounded-full flex place-content-center place-items-center py-1 text-white font-semibold"
                >
                  {item.name}
                </div>
              </li>
